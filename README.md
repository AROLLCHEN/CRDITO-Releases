<div align="center">
  ...
  <p>
    <a href="https://github.com/AROLLCHEN/CRDITO-Releases/releases/download/v1.0.10/CRDITO-Setup-1.0.10-win-x64.exe"><strong>下载 CRDITO 1.0.10</strong></a>
    ·
...
- 轻量模式会在器件名称和校验状态之间显示“数据手册”入口；仅当当前器件具有有效链接时显示，点击后使用默认浏览器打开。
...
3. 切换到 AD，在原理图画布内双击后开始放置；器件会附着到鼠标并使用工程目录中的统一 SchLib/PcbLib 关联封装。
4. 需要重复放置同一器件时，在“设置”中启用“连续放置同一器件”。每次确认后会自动生成下一个待放置器件，按 `Esc` 结束连续放置。
5. 在“设置 → 按 Esc 时”选择“删除当前器件”或“仅结束放置”，即可控制取消行为。

> 提示：按住 `Ctrl` 点击顶部可配置的菜单项，可以设置快捷键；重复快捷键会在保存前提示冲突。“窗口置顶”和“置顶时进入轻量模式”可让器件选择和连续放置保持在更紧凑的操作界面中。
...
| 连续放置 | 可选地在每次确认后自动创建同一器件的下一次放置，按 `Esc` 结束 |
